...
namespace UZeroConsole.WebConsole
{
    ...
    public class PerformanceTracker
    {
        ...
        PerformanceCounter counter_WebService_CurrentConnections;
        ...
        public PerformanceTracker(int MiliSeconds = 100, bool EnergySaver = true, float CPU_Critical_Limit = 90, float RAM_Critical_limit = 90)
        {
            ...
            counter_WebService_CurrentConnections = new PerformanceCounter("Web Service", "Current Connections", "_Total");
        ...
        private void Monitor(int MiliSeconds)
        {
            ...
            while (true)
            {
                ...
                WebService_CurrentConnections = counter_WebService_CurrentConnections.NextValue();

                //For Testing Purposes-->
                Console.WriteLine("CPU: " + Math.Round(this.Current_CPU_Usage, 2, MidpointRounding.AwayFromZero) + "%");
                ...
                Console.WriteLine("Memory: " + Math.Round(this.Percent_RAM_Used, 2, MidpointRounding.AwayFromZero) + "%");
                foreach (var deskPercent in Percent_DiskSpace_Available)
                {
                    Console.WriteLine(deskPercent);
                }
                Console.WriteLine(string.Format("WebService.CurrentConnections: {0}", WebService_CurrentConnections));
                Console.WriteLine();
        ...
        private List<string> DiskSpaceAvailable()
        {
            ...
            foreach (DriveInfo drive in drives)
            {
                ...
                if (drive.IsReady)
                {
                    //double a = drive.TotalFreeSpace / (double)drive.TotalSize;
                    double availableSpace = (((double)drive.TotalFreeSpace / (double)drive.TotalSize) * 100);
                    //+" :: " + Math.Round(availableSpace, 2, MidpointRounding.AwayFromZero) + "%"
                    var total = drive.TotalSize / 1024 / 1024 / 1024;
                    var free = drive.TotalFreeSpace / 1024 / 1024 / 1024;

                    DiskInfo.Add(string.Format("{0}: 总数{1},已使用{2}，空闲{3}",
                        drive.Name,
                        (total) + "GB",
                        (total - free) + "GB",
                        (free) + "GB"
