...
{
    public partial class SystemPerformance : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //Thread.Sleep(2000);
            //WriteLine("CPU", thisDevice.Current_CPU_Usage.ToString());
            //thisDevice.Stop();

            //Disks.Add("123");
            //Disks.Add("234");
            //Response.Write(Newtonsoft.Json.JsonConvert.SerializeObject(Disks));

            Response.Write(GetTotalRAMInMB());

            //Response.Write(cpuCounter.NextValue());
        }
        public List<string> Disks { get; } = new List<string>();

        private void Btn_Click(object sender, EventArgs e)
        {
            WriteLine("CPU", PerformanceCounterFun("Processor", "_Total", "% Processor Time").ToString());
        }

        private void WriteLine(string name, string msg)
        {
            ltlBod.Text = string.Format("{0}：{1} <br />", name, msg);
        }
