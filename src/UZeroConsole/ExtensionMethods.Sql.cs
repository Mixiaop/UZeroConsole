    ...
    public static partial class ExtensionMethods
    {
        ...
        private static string MarkSqlString(string sql, string path, int lineNumber, string comment)
        {
            ...
            unchecked
            {
                ...
                if (comment.IsNotNullOrEmpty()) key = (key * 23) + comment.GetHashCode();
            }
            ...
            var ret = sql.Substring(0, i) + " /* " + path.Substring(split) + "@" + lineNumber.ToString() + (comment.IsNotNullOrEmpty() ? " - " + comment : "") + " */" + commentWrap + sql.Substring(i);
