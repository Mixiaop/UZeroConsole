...
namespace UZeroConsole.Monitoring.Redis
{
    public class RedisModule : StatusModule
    {
        ...
        private static List<RedisConnectionInfo> LoadRedisConnections()
        {
            ...
            var defaultServerInstances = Current.MonitoringSettings.Redis.Defaults.Instances;
            var allServerInstances = Current.MonitoringSettings.Redis.AllServers.Instances;

            foreach (var s in Current.MonitoringSettings.Redis.Servers)
            {
